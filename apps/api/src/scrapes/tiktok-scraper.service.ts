...
export class TikTokScraperService {
  detectType(url: string): ScrapeType {
    try { const u = new URL(url); if (!/(^|\.)tiktok\.com$/i.test(u.hostname.replace(/^www\./, ''))) throw new Error(); return /\/(video|photo)\/\d+/.test(u.pathname) ? 'VIDEO' : 'PROFILE'; }
  ...
  async scrape(url: string): Promise<Metrics> {
    ...
    let html: string;
    try { const r = await fetch(url, { headers: { 'user-agent': 'Mozilla/5.0' }, signal: AbortSignal.timeout(12_000), redirect: 'follow' }); if (!r.ok) throw new Error(String(r.status)); html = await r.text(); }
      ...
      if (type === 'VIDEO') {
        try {
          ...
          if (apiData.code === 0 && apiData.data) {
            return { type, views: this.num(apiData.data.play_count), likes: this.num(apiData.data.digg_count), comments: this.num(apiData.data.comment_count), shares: this.num(apiData.data.share_count) };
          }
        } catch (e) {}
      }
